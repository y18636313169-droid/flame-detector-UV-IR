/**
  ******************************************************************************
  * @file    hal_uart.h
  * @brief   BSP UART 抽象层 — DMA 环形缓冲驱动
  *
  *          TX: 应用写入环形缓冲 → DMA 自动搬运。发送完成中断自动链式发送下一段。
  *          RX: DMA 循环模式连续填充缓冲 → 应用读取/Peek → 更新读指针。
  *
  *          串口资源：
  *            COM (通信) = USART1 — PA9(TX)/PA10(RX), DMA1_CH4(TX)/CH5(RX) — CubeMX 管理
  *            DBG (调试) = USART2 — PA2(TX)/PA3(RX),  DMA1_CH7(TX)/CH6(RX) — CubeMX 管理
  ******************************************************************************
  */
...
#define BSP_UART_COM_INST           USART1    /* 通信串口（外部协议交互） */
#define BSP_UART_DBG_INST           USART2    /* 调试串口（日志打印）     */
